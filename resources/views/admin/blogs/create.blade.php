...
@section('title', 'Create Safari Story - Advanced Editor')
...
<div class="min-h-screen bg-slate-900 py-8" data-theme="dark" id="app-container">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" style="padding-bottom: 180px;">
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-white">Create Safari Story</h1>
                    <p class="text-sm text-slate-400 mt-1">✨ Nested Tags • Click Inside DIV to Insert Inside • Double-Click to Exit</p>
                </div>
            </div>
                ...
                <div class="lg:col-span-2 space-y-5">
                    ...
                    <div class="bg-slate-800 rounded-xl shadow-sm border border-slate-700 p-5">
                        ...
                        <div class="mt-2 flex items-center text-sm text-slate-400">
                            ...
                            <span id="slug-preview" class="font-mono text-indigo-400 ml-1 bg-slate-900 px-2 py-1 rounded">{{ Str::slug(old('title', 'your-post-slug')) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-800 rounded-xl shadow-sm border border-slate-700 p-5">
                        ...
                        <textarea 
                            ...
                            placeholder="A brief summary...">{{ old('excerpt') }}</textarea>
                    </div>
                        ...
                        <div id="seo-panel" class="p-5 space-y-4 hidden">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input id="meta_title" name="meta_title" value="{{ old('meta_title') }}" class="block w-full bg-slate-900 border border-slate-600 text-white rounded-lg shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 text-sm px-3 py-2">
                                ...
                                <div>
                                    ...
                                    <input id="meta_keywords" name="meta_keywords" value="{{ old('meta_keywords') }}" class="block w-full bg-slate-900 border border-slate-600 text-white rounded-lg shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 text-sm px-3 py-2" placeholder="safari, wildlife, kenya">
                                </div>
                            </div>
                            <div>
                                <label for="meta_description" class="block text-xs font-medium text-slate-400 mb-1">Meta Description</label>
                                <textarea id="meta_description" name="meta_description" rows="2" class="block w-full bg-slate-900 border border-slate-600 text-white rounded-lg shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 text-sm px-3 py-2">{{ old('meta_description') }}</textarea>
                    ...
                    <div class="bg-slate-800 rounded-xl shadow-sm border border-slate-700 overflow-hidden sticky top-6">
                        ...
                        <div class="p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="author_name" name="author_name" value="{{ old('author_name') }}" class="mt-1 block w-full bg-slate-900 border border-slate-600 text-white rounded-lg shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 px-3 py-2">
                            </div>
                            ...
                            <div id="scheduled-datetime" class="{{ old('status') === 'scheduled' ? '' : 'hidden' }}">
                                ...
                                <input id="published_at" name="published_at" value="{{ old('published_at') }}" class="mt-1 block w-full bg-slate-900 border border-slate-600 text-white rounded-lg shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 px-3 py-2" type="datetime-local">
                            </div>

                            {{-- Tags --}}
                            <div>
                                <label for="tags" class="block text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">Tags</label>
                                <input id="tags" name="tags" value="{{ old('tags') }}" class="mt-1 block w-full bg-slate-900 border border-slate-600 text-white rounded-lg shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 px-3 py-2" placeholder="safari, wildlife, kenya">
                            </div>

                            {{-- Featured Toggle --}}
                          <div class="flex items-center justify-between p-3 bg-yellow-900 bg-opacity-20 rounded-lg border border-yellow-700">
    <div class="flex items-center space-x-2">
        <svg class="w-5 h-5 text-yellow-400" fill="{{ old('is_featured') ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <label class="relative inline-flex items-center cursor-pointer">
        <!-- THIS FIXES THE ISSUE - sends false when unchecked -->
        <input type="hidden" name="is_featured" value="0">
        <input type="checkbox" id="is_featured" name="is_featured" class="sr-only peer" value="1" {{ old('is_featured') ? 'checked' : '' }}>
...
(function() {
    ...
    console.log('🚀 Editor initializing with NESTED TAG support...');
    ...
    const AUTOSAVE_KEY = 'safari_blog_autosave';
    ...
    // ==========================================================================
    // 1) THEME SWITCHER - FIXED: Applies to entire page
    // ==========================================================================
    function setupThemeSwitcher() {
        ...
        function applyTheme(theme) {
            ...
            document.querySelectorAll('.bg-slate-800, .bg-slate-900, .bottom-toolbar').forEach(el => {
    ...
    // ==========================================================================
    // 2) CREATE TAG WRAPPER - FIXED: Proper nested structure
    // ==========================================================================
    function createTagWrapper(tagName, initialContent = '') {
        ...
        wrapper.setAttribute('contenteditable', 'false'); // Parent not editable
    ...
    // ==========================================================================
    // 7) INSERT AT CURSOR OR INSIDE FOCUSED PARENT - FIXED: NESTED INSERTION
    // ==========================================================================
    function insertAtCursor(element) {
        if (!editor) return;
        
        const selection = window.getSelection();
        let parentElement = editor;
        
        // Check if we're inside a tag-content div (which means we're inside a wrapper)
        if (selection.rangeCount > 0) {
            ...
            // Find if we're inside a tag-content element
            const tagContent = container.closest('.tag-content');
            if (tagContent) {
                // We're inside an existing tag - insert INSIDE this tag's content
                const parentWrapper = tagContent.closest('.tag-wrapper');
                
                // Create a new line/paragraph inside the current content
                ...
                // Now insert the new element after the current position
                if (br.nextSibling) {
    ...
    // ==========================================================================
    // 10) DOUBLE CLICK TO EXIT - FIXED: Proper focus movement
    // ==========================================================================
// ==========================================================================
// 10) DOUBLE CLICK TO EXIT - FIXED: ONLY changes focus, NO new tag added
// ==========================================================================
function setupDoubleClickExit() {
    editor.addEventListener('dblclick', function(e) {
        const target = e.target;
        
        // If double-clicked on editable content, move focus out of the wrapper
        if (target.classList.contains('tag-content') || 
            ...
            if (wrapper) {
                // Find the parent wrapper to move focus to
                const parentWrapper = wrapper.parentNode.closest?.('.tag-wrapper') || null;
                
                if (parentWrapper) {
                    // We're inside a nested wrapper - move focus to parent's content area
                    ...
                    // We're at root level - move focus to root editor
                    editor.focus();
    ...
    function loadFromLocalStorage() {
        ...
        try {
            ...
            const restore = confirm(`Found auto-saved draft from ${new Date(data.timestamp).toLocaleString()}. Restore it?`);
            if (!restore) {
            ...
            if (authorInput) authorInput.value = data.author_name || '';
            ...
            if (featuredCheckbox) featuredCheckbox.checked = data.is_featured || false;
    ...
    // ==========================================================================
    function init() {
        console.log('🚀 Starting initialization with NESTED TAG support...');
        
        const restored = loadFromLocalStorage();
        
        if (!restored && (!editor.innerHTML || editor.innerHTML.trim() === '')) {
        ...
        console.log('✅ Editor fully initialized with NESTED TAG support!');
        console.log('📝 Tip: Click INSIDE any DIV to insert new tags INSIDE it');
    }
