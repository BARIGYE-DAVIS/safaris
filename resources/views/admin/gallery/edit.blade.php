    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white shadow rounded-lg overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    <div class="mt-4 flex space-x-2">
                        <form action="{{ route('admin.gallery.toggle-visibility', $gallery->id) }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" 
                                    class="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-md {{ $gallery->is_visible ? 'text-red-700 hover:bg-red-50' : 'text-green-700 hover:bg-green-50' }} transition-colors">
                                {{ $gallery->is_visible ? 'Hide' : 'Show' }}
                            </button>
                        </form>

                        <form action="{{ route('admin.gallery.destroy', $gallery->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this image? This action cannot be undone.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="flex-1 px-3 py-2 text-sm border border-red-300 text-red-700 rounded-md hover:bg-red-50 transition-colors">
                                Delete
                            </button>
                        </form>
                    </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white shadow rounded-lg">
                ...
                <form action="{{ route('admin.gallery.update', $gallery->id) }}" method="POST" enctype="multipart/form-data" class="px-6 py-6 space-y-6" id="galleryForm">
                    @csrf
                    @method('PUT')
