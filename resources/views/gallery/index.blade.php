...
@section('meta_description', 'Explore our breathtaking safari photo gallery featuring stunning wildlife photography, African landscapes, and unforgettable safari moments. View high-quality images from East Africa\'s premier safari destinations.')
@section('meta_keywords', 'safari photos, wildlife photography, African wildlife, safari gallery, nature photography, Kenya safari, Tanzania safari, wildlife images, safari pictures, African landscapes')
...
<script type="application/ld+json">
{
    ...
    "@type": "ImageGallery",
    "name": "Safari Photo Gallery - Calm Africa Safaris",
    "description": "Professional safari photography showcasing Africa's incredible wildlife and landscapes",
    "url": "{{ url()->current() }}",
    "provider": {
        "@type": "Organization",
    ...
    "image": [
        @foreach($images->take(10) as $image)
        {
            ...
            "datePublished": "{{ $image->created_at->toISOString() }}",
            "creator": {
                "@type": "Organization",
                "name": "Calm Africa Safaris"
            }
...
<meta property="og:title" content="Safari Photo Gallery - Stunning Wildlife Photography">
<meta property="og:description" content="Explore our breathtaking safari photo gallery featuring stunning wildlife photography and African landscapes from premier safari destinations.">
<meta property="og:image" content="{{ $images->first()->image_url ?? '' }}">

<!-- Twitter -->
<meta property="twitter:card" content="summary_large_image">
<meta property="twitter:url" content="{{ url()->current() }}">
<meta property="twitter:title" content="Safari Photo Gallery - Wildlife Photography">
<meta property="twitter:description" content="Stunning safari photography from East Africa's premier wildlife destinations.">
<meta property="twitter:image" content="{{ $images->first()->image_url ?? '' }}">
@endpush
...
<div class="bg-gradient-to-r from-green-800 via-green-700 to-green-600">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
        ...
        <p class="text-xl md:text-2xl text-green-100 mb-8 max-w-3xl mx-auto">
            ...
            From majestic lions to spectacular landscapes, discover the magic of safari adventures.
        </p>
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('gallery.index') }}" 
                       class="px-4 py-2 rounded-full text-sm font-medium transition-colors bg-green-600 text-white">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-2">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                    Search: "{{ request('search') ?? 'All Images' }}"
                    <a href="{{ route('gallery.index') }}" class="ml-2 text-blue-600 hover:text-blue-800">
                ...
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
                    Category: {{ ucfirst(str_replace('_', ' ', request('category') ?? 'all')) }}
                    <a href="{{ route('gallery.index') }}" class="ml-2 text-green-600 hover:text-green-800">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            <a href="{{ route('gallery.index') }}" class="text-sm text-gray-600 hover:text-gray-800">
                Clear all filters
            </a>
        </div>
    </div>
            ...
            <article class="group relative bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                ...
                <div class="aspect-w-1 aspect-h-1 relative overflow-hidden">
                    ...
                    <div class="absolute top-3 left-3">
                        <span class="inline-block bg-green-600/90 backdrop-blur-sm text-white text-xs font-medium px-2 py-1 rounded-full">
                            {{ $image->category_label ?? 'Uncategorized' }}
                        </span>
                    </div>
                </div>
                ...
                <div class="p-4">
                    ...
                    <p class="text-gray-600 text-sm mb-3 line-clamp-2">
                        {{ $image->caption ?? 'Beautiful safari moment' }}
                    </p>
                    ...
                    <div class="flex flex-wrap gap-1 mb-3">
                        ...
                        @endforeach
                        <span class="text-xs text-gray-400">{{ count($image->formatted_tags ?? []) > 3 ? '+' . (count($image->formatted_tags) - 3) : '' }}</span>
                    </div>
    ...
    <!-- Empty State (only shows when no images) -->
    <div class="text-center py-16" style="display: {{ $images->count() > 0 ? 'none' : 'block' }};">
        ...
        <p class="text-gray-600 mb-6">Our photo gallery is currently being updated. Check back soon for stunning safari imagery!</p>
    </div>
</div>
